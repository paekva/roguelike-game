...
class Squad {
	...
	attack_squad(target_squad, range, tgt_tile) {
		...
		for (let unit of this.units) {
			if (
				unit.status === 'Alive' &&
				target_squad.units.length !== 0 &&
				unit.weapon.max_range >= range &&
				unit.weapon.min_range <= range &&
				unit.weapon.ammo !== 0
			) {
				if (unit.weapon.type === 'Assault') {
					for (let i = 0; i < unit.weapon.attacks; i++) {
						unit.attack_unit(
							target_squad.get_target_priorities(),
							tgt_tile
						)
	...
	BFS(start, battlefield, mode) {
		...
		while (toExplore.length > 0) {
			...
			for (let neighbor of battlefield[tileIndex.X][tileIndex.Y]
				.neighbors) {
						...
						if (mode === 1) {
							if (
								...
								distance[tileIndex.X][tileIndex.Y] +
									battlefield[tileIndex.X][tileIndex.Y]
										.mp_required
							) {
								distance[neighbor.X][neighbor.Y] =
									distance[tileIndex.X][tileIndex.Y] +
