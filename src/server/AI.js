class AI {
	constructor(battlefield, behavior) {
		this.desires = [];
		...
		this.modifiers = { attack: 1, flee: 0, scout: 0.5 };
	...
	init_desire_map(battlefield, desire_map, invert, fade) {
		for (let i = 0; i < battlefield.length; i++) {
			for (let j = 0; j < battlefield[0].length; j++) {
				desire_map[i][j] = 0;
				if (battlefield[i][j].unit) {
					if (this.vis_map[i][j] !== 0) {
						if (battlefield[i][j].unit.energy) {
							if (invert === false) {
								desire_map[i][j] = fade;
							}
							if (invert === true) {
	...
	// 			if (battlefield[i][j].unit) {
	// 				if (battlefield[i][j].unit.owner === this.player) {
				...
				if (battlefield[i][j].is_passable) {
					for (let neighbor of neighbors) {
						if (invert === false) {
							...
							if (
								desire_map[neighbor.X][neighbor.Y] - 1 * strength >
								desire_map[i][j]
							) {
								desire_map[i][j] =
									desire_map[neighbor.X][neighbor.Y] - 1 * strength;
	...
	// 		for (let j = 0; j < battlefield[0].length; j++) {
	// 			let desirability =
	// 				(battlefield[i][j].eva_bonus - (battlefield[i][j].eva_bonus % 5)) / 5;
	...
	fill_final_desire_map(battlefield) {
		for (let i = 0; i < battlefield.length; i++) {
			for (let j = 0; j < battlefield[0].length; j++) {
				this.final_desire_map[i][j] =
					this.desire_map_attack[i][j] * this.modifiers.attack +
					this.desire_map_flee[i][j] * this.modifiers.flee +
					// this.desire_map_stay_in_cover[i][j] * this.modifiers.cover +
					// this.desire_map_stay_close_to_allies[i][j] * this.modifiers.allies +
					this.desire_map_scout[i][j] * this.modifiers.scout;
			}
		}
	}
...
let unit_AI = null;
module.exports.new = function(battlefield, behavior) {
	new_unit_AI = new AI(battlefield, behavior);
	unit_AI = new_unit_AI;
	return new_unit_AI;
};

module.exports.update_desire_map = function(battlefield, X, Y, visib_map) {
	unit_AI.update_desire_map(battlefield, X, Y, visib_map);
};

module.exports.BFS = function(start, battlefield, mode) {
	unit_AI.BFS(start, battlefield, mode);
};
