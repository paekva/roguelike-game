...
class battle_controller {
	...
	move_squad_to_tile(squad_info, goal, res) {
		...
		let prevY = squad.Y;
		if (
			this.battlefield[goal.X][goal.Y].squad === null &&
			this.battlefield[goal.X][goal.Y].is_passable &&
			this.players[squad_info.player].squads[squad_info.index].cur_movement >=
				res.cost
		) {
			this.players[squad_info.player].squads[squad_info.index].cur_movement -=
				res.cost;
			...
			this.battlefield[goal.X][goal.Y].squad = this.players[
				squad_info.player
			].squads[squad_info.index];
