...
io.sockets.on('connection',
  function (socket) {
    ...
    function (data) {
      ...
      if (data === 'up') {
        result = controller.hero.move_unit(controller.battlefield[controller.hero.X][controller.hero.Y - 1]) 
      }
      if (data === 'down') {
        result = controller.hero.move_unit(controller.battlefield[controller.hero.X][controller.hero.Y + 1]) 
      }
                ...
                for (let i = 0 ; i < controller.battlefield.length; i++) {
                    for (let j = 0 ; j < controller.battlefield[0].length; j++) {
                        if (distances[i][j] <= unit.cur_movement) {
                            in_potential_range.push({X: i, Y: j, priority: unit.AI.final_desire_map[i][j]})
